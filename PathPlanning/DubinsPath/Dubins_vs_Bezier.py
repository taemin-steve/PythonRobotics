...
def bezier_curve(control_points, num_points=100):
    ...
    t = np.linspace(0, 1, num_points).reshape(num_points, 1)  # t를 열 벡터로 변환
    curve = np.zeros((num_points, 2))
    for i in range(n + 1):
        binomial_coeff = math.comb(n, i)
        term = binomial_coeff * (t ** i) * ((1 - t) ** (n - i))
...
#------------------------------------------------------
# 경로점 설정
def calculate_and_update_angles(two_dim_waypoints):
    
    if len(two_dim_waypoints) < 2:
      raise ValueError("두 개 이상의 경로점이 필요합니다.")
  
    for i in range(len(two_dim_waypoints)):
        if i == len(two_dim_waypoints)- 1:
            waypoints.append((two_dim_waypoints[i][0], two_dim_waypoints[i][1],waypoints[-1][2]))
            break
        previous_waypoint = two_dim_waypoints[i]
        current_waypoint = two_dim_waypoints[i + 1]
        ...
        # waypoints 리스트 업데이트
        waypoints.append((previous_waypoint[0], previous_waypoint[1], np.deg2rad(angle)))
...
## 실제 그려질 waypoints (x,y,yaw) 형식
waypoints = []

calculate_and_update_angles(two_dim_waypoints)


dubins_path_x = np.array([])
dubins_path_y = np.array([])

#---------------------------------------------------------
# 실제 계산

# Dubins path 계산s
curvature = 0.5 # 곡률 // 수상정의 물리적 능력에 따라 조절이 필요
for i in range(len(waypoints)- 1):
    x, y, _, _, _ = plan_dubins_path(waypoints[i][0], waypoints[i][1], waypoints[i][2], waypoints[i+1][0], waypoints[i+1][1], waypoints[i+1][2], curvature)
    dubins_path_x = np.concatenate((dubins_path_x, x))
    dubins_path_y = np.concatenate((dubins_path_y, y))
    
# 베지에 곡선 계산
bezier_path_x, bezier_path_y = bezier_curve([wp[:2] for wp in waypoints])

# --------------------------------------------------------
# 시각화
plt.figure(figsize=(10, 5))
plt.plot(dubins_path_x, dubins_path_y, label="Dubins Path", linestyle='-')
plt.plot(bezier_path_x, bezier_path_y, label="Bezier Curve", linestyle='-')

# 웨이포인트 시각화
for i, (x, y, _) in enumerate(waypoints):
    plt.scatter(x, y, color='red')  # 웨이포인트에 빨간색 점 추가
    plt.text(x, y, f'WP{i+1}', color='blue', fontsize=9)  # 각 웨이포인트 옆에 레이블 추가


plt.plot([], [], ' ', label=f'Curvature: {curvature}')


plt.legend()
plt.grid(True)
plt.axis('equal')
plt.title('Path Comparison between Dubins Path and Bezier Curve')
plt.show()
